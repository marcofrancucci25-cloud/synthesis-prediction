...
## v10.11.2 — coherent ligand/metal sensitivity

- Removed the artificial `0.1` L:M perturbation generated by subtracting one from a current 1:1 ratio.
- Ratio alternatives now come from the central 90% of ratios actually observed, prioritizing the same ligand–metal system, then the same metal and ligand family.
- Every ratio perturbation preserves total precursor amount and recalculates both ligand and metal mmol, so `Rapporto_LM` can no longer contradict the entered quantities.
